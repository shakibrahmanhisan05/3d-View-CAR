...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return { title: dict.nav.demoBike, description: dict.twoProducts.newBody };
}

export default async function BikeDemoPage({
  params,
  searchParams,
}: {
  ...
  const vehicle = getVehicleBySegment('motorcycle');
  if (!vehicle) notFound();
